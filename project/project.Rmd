...
pc_analysis2 <- movies %>%
  filter(major_productionco %in% c("pixar", "marvel", "20thc", "dreamworks", "universal")) %>%
  group_by(major_productionco, year) %>%
  summarise(median = median(pratio))

ggplot(data = pc_analysis2, aes(x = year, y = median, color = major_productionco, group = major_productionco)) +
  geom_line()
```
